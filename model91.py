...
class SRCNN:
    ...
    def network(self):
        ...
        conv_w = tf.get_variable("conv_8_w", [3,3,nbr_patches,3], initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0/9/64)))
        conv_b = tf.get_variable("conv_8_b", [3], initializer=tf.constant_initializer(0))
        weights.append(conv_w)
        weights.append(conv_b)
